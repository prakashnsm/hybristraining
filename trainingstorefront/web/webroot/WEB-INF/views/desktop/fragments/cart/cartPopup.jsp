	...
	<c:forEach items="${entries}" var="entry" end="${numberShowing - 1}">
		...
		<li class="cart_modal_popup">
			...
			<div class="prod_info">
				...
				<p class="prod_quantity"><span class="product-quantity-label"><spring:theme code="popup.cart.quantity"/></span>${entry.quantity}</p>
				...
				<c:if test="${not empty entry.deliveryPointOfService.name}">
					<p class="prod_pickup"><span class="product-pickup-label"><spring:theme code="popup.cart.pickup"/></span>${entry.deliveryPointOfService.name}</p>
				</c:if>
			</div>
		</li>
	</c:forEach>
