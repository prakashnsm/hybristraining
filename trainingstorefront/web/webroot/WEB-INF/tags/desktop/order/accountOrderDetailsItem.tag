...
<div class="item_container_holder">
	...
	<c:choose>
			<c:when test="${not inProgress}">
				<h3><spring:theme code="text.account.order.consignment.status.${consignment.statusDisplay}"/>: <fmt:formatDate value="${consignment.statusDate}" pattern="MM/dd/yy"/></h3>
				<c:choose>
		...
		<c:choose>
			<c:when test="${consignment.status.code eq 'READY_FOR_PICKUP'}" >
				...
				<div class="ui-grid-a productItemFullPusher" data-theme="b">
					<h4>Pick Up Location: ${pos.name}, ${pos.address.line1} ${pos.address.line2}, ${pos.address.town}</h4>
				</div>
			</c:when>
		</c:choose>
