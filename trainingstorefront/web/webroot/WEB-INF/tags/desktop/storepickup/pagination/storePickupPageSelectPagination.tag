...
<c:if test="${hasNextPage}">
	<form action="${pickupInStoreUrl}" id="last_page_storepickup_form" method="get">
		...
		<button class="neutral paginate_next_pickupstores_button" type="submit"><spring:theme code="pickup.pagination.last"/></button>
	</form>
...
<c:forEach begin="${beginPage}" end="${endPage}" var="page">
	...
	<c:choose>
		<c:when test="${searchPageData.pagination.currentPage+1 eq pageNumber}">
			<div><button class="positive selected_page_pickupstores_button" type="button" disabled="disabled">${pageNumber}</button></div>
		</c:when>
		<c:otherwise>
			<form action="${pickupInStoreUrl}" id="page_${pageNumber}_storepickup_form" method="get">
				...
				<button class="neutral selected_page_pickupstores_button" type="submit">${pageNumber}</button>
			</form>
		</c:otherwise>
	</c:choose>
</c:forEach>
<c:if test="${searchPageData.pagination.currentPage gt 0}">
	...
	<form action="${pickupInStoreUrl}" id="first_page_storepickup_form" method="get">
		...
		<button class="neutral paginate_next_pickupstores_button" type="submit"><spring:theme code="pickup.pagination.first"/></button>
	</form>
</c:if>