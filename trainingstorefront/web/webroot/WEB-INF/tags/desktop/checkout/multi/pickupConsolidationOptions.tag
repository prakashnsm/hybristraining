...
<c:if test="${not empty pickupConsolidationOptions}">
	...
	<div class="span-24">
		<div class="selectDeliverylocation clearfix item_container_holder">
			<form:form id="simplifyDeliverylocationForm" action="${consolidatePickupUrl}" method="POST">
				...
				<c:forEach items="${pickupConsolidationOptions}" var="option" varStatus="status">
					<div class="selectDeliverylocationItem">
						<input id="selectDeliverylocationItemId${status.count}" class="selectDeliverylocationItemOption" type="radio" name="posName" value="${option.name}" <c:if test="${status.first}">checked="checked"</c:if>/>
						<label class="selectDeliverylocationItemLabel" for="selectDeliverylocationItemId${status.count}">
				...
				<div class="selectDeliverylocationItem">
					<spring:theme code="checkout.pickup.items.simplify.pickup.location" />
