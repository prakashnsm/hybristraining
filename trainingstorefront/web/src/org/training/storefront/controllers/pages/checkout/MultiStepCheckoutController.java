...
import org.training.storefront.breadcrumb.ResourceBreadcrumbBuilder;
...
import org.training.storefront.forms.SopPaymentDetailsForm;
import org.training.storefront.forms.address.verification.AddressVerificationResultHandler;
import org.training.storefront.forms.validation.AddressValidator;
...
public class MultiStepCheckoutController extends AbstractCheckoutController
{
	...
	{
		final List<CheckoutSteps> checkoutSteps = new ArrayList<>();
	...
	public String gotoFirstStep()
	{
		if (hasValidCart())
		{
			if (hasShippingItems())
			{
				// user has already gone through the checkout steps
				if (!hasNoDeliveryAddress() && !hasNoDeliveryMode() && !hasNoPaymentInfo())
				{
	...
	{
		return ControllerConstants.Views.Pages.MultiStepCheckout.CheckoutLoginPage; // TODO: Add login page view with 3 login options
	}
	...
	@RequireHardLogIn
	public String doChooseDeliveryLocation(final Model model) throws CMSItemNotFoundException
	{
		if (!hasValidCart())
		{
			LOG.info("Missing, empty or unsupported cart");
			...
			CommerceCartModificationException
	{
		...
		if (!hasPickUpItems() && cartData.getDeliveryMode().getCode().equals("pickup"))
		{
			return REDIRECT_URL_CHOOSE_DELIVERY_METHOD;
		}
			...
			InvalidCartException, CommerceCartModificationException
	{
		...
		if (!getCheckoutFacade().authorizePayment(placeOrderForm.getSecurityCode()))
		{
